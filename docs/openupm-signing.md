...
This document describes how `IvanMurzak/Unity-Gyroscope-Parallax` signs its
`extensions.unity.gyroscope.parallax` package so the warning no longer appears in
Unity 6.3+.
...
with `extensions.unity.gyroscope.parallax-`, and uploads the signed `.tgz` as a
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-Gyroscope-Parallax
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-Gyroscope-Parallax
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-Gyroscope-Parallax
```

### 3. File the OpenUPM listing change

OpenUPM's package listing for `extensions.unity.gyroscope.parallax` currently has
...
repository at `data/packages/extensions.unity.gyroscope.parallax.yml`. Open a PR
there changing:
...
1. Go to the [release page](https://github.com/IvanMurzak/Unity-Gyroscope-Parallax/releases)
   for the new version and confirm an `extensions.unity.gyroscope.parallax-<version>.tgz`
...
2. Inspect the tarball locally to confirm it contains the signing attestation:
   ...
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-Gyroscope-Parallax/releases/download/<version>/extensions.unity.gyroscope.parallax-<version>.tgz
